export const sectionsData =

{
    monihomes: [
        {
            'image': require('./images/concept/monihomes/monihomes_night.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room1_4.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_1.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_2.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_3.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_4.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_5.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_6.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room2_7.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room3_2.jpg')
        },
        {
            'image': require('./images/concept/monihomes/room3_3.jpg')
        },

    ],
    villamasal: [
        {
            'image': require('./images/concept/villamasal/villamasal_3.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_1.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_2.jpg')
        },

        {
            'image': require('./images/concept/villamasal/villamasal_4.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_5.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_6.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_7.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_8.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_9.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_10.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_11.jpg')
        },
        {
            'image': require('./images/concept/villamasal/villamasal_12.jpg')
        },

    ],
    yoyo_tinyhouse: [
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_1.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_2.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_3.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_4.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_5.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_6.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_7.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_8.jpg')
        },
        {
            'image': require('./images/concept/yoyotinyhouse/yoyo_9.jpg')
        },

    ],
    fethiye: [
        {
            'image': require('./images/area/fethiye/DSC_0013.JPG')
        },
        {
            'image': require('./images/area/fethiye/DSC_0187.JPG')
        },
        {
            'image': require('./images/area/fethiye/DSC_0189.JPG')
        },
        {
            'image': require('./images/area/fethiye/DSC_0397.JPG')
        },
        {
            'image': require('./images/area/fethiye/DSC_0399.JPG')
        },
        {
            'image': require('./images/area/fethiye/IMG_20150301_150531.jpg')
        },

    ],
    restaurants: [
        {
            'image': require('./images/area/restaurant/restaurant_1.jpg')
        },
        {
            'image': require('./images/area/restaurant/stores_4.jpg')
        },
        {
            'image': require('./images/area/restaurant/restaurant_2.jpg')
        },
        {
            'image': require('./images/area/restaurant/stores_2.png')
        },
        {
            'image': require('./images/area/restaurant/restaurant_3.jpg')
        },
        {
            'image': require('./images/area/restaurant/stores_1.jpg')
        },

        {
            'image': require('./images/area/restaurant/stores_3.jpg')
        },

    ],
    experiences: [
        {
            'image': require('./images/area/nature/paragliding_2.jpg')
        },
        {
            'image': require('./images/area/nature/likya.jpg')
        },
        {
            'image': require('./images/area/nature/The_Lycian_Way.gif')
        },
        {
            'image': require('./images/area/nature/paragliding.JPG')
        },
        {
            'image': require('./images/area/nature/likya_2.jpg')
        },
        {
            'image': require('./images/area/nature/paragliding_3.jpg')
        },
        {
            'image': require('./images/area/nature/scuba.jpg')
        },

    ],
    goforadrive: [
        {
            'image': require('./images/area/gofordrive/horse_riding.jpg')
        },
        {
            'image': require('./images/area/gofordrive/safari_2.jpg')
        },
        {
            'image': require('./images/area/gofordrive/quad.png')
        },
        {
            'image': require('./images/area/gofordrive/safari.png')
        },
        {
            'image': require('./images/area/gofordrive/quad_2.png')
        },
    ],
    beaches: [
        {
            'image': require('./images/area/beaches/belcekiz.jpg')
        },
        {
            'image': require('./images/area/beaches/boncuklu.png')
        },
        {
            'image': require('./images/area/beaches/calis.png')
        },
        {
            'image': require('./images/area/beaches/gunluklu.jpg')
        },
        {
            'image': require('./images/area/beaches/katranci.jpg')
        },
        {
            'image': require('./images/area/beaches/boat_trip.jpg')
        },
        {
            'image': require('./images/area/beaches/kelebekler_vadisi.jpg')
        },
        {
            'image': require('./images/area/beaches/kuleli.jpg')
        },
        {
            'image': require('./images/area/beaches/boat_trip_2.jpeg')
        },
        {
            'image': require('./images/area/beaches/kumburnu.jpg')
        },
    ],
    aboutus: [
        {
            'image': require('./images/aboutus/restaurant_black_white.jpg')
        },
        {
            'image': require('./images/aboutus/panaroma.jpg')
        },
        {
            'image': require('./images/aboutus/villa_masal.jpg')
        },
        {
            'image': require('./images/aboutus/monihomes.jpg')
        },
    ]

}

